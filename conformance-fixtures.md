...
This document is the **catalogue**: it lists each conformance fixture, the MCLIP rule(s) it exercises, the synthetic MCP server shape needed, the client invocation, and the expected pass criteria. The fixtures **themselves** (the synthetic MCP servers under `fixtures/servers/` and the harness under `fixtures/verify/`) are a separate deliverable specified in `fixtures-spec.md`.
